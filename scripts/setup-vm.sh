...
if [ -f "${ROOT}/ci/.env" ]; then
    echo ""
    echo " Jenkins ja vem com o usuario admin e o job 'task2-pipeline'"
    echo " criados (leva ~1 min para ficar pronto). Credenciais:"
    echo "   Usuario: $(grep '^JENKINS_ADMIN_USER=' "${ROOT}/ci/.env" | cut -d= -f2)"
    echo "   Senha:   $(grep '^JENKINS_ADMIN_PASSWORD=' "${ROOT}/ci/.env" | cut -d= -f2)"
fi
echo ""
echo " Tudo pronto: basta logar no Jenkins e rodar o 'task2-pipeline'."
echo "=============================================="
